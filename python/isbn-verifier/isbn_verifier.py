...
(d₁ * 10 + d₂ * 9 + d₃ * 8 + d₄ * 7 + d₅ * 6 + d₆ * 5 + d₇ * 4 + d₈ * 3 + d₉ * 2 + d₁₀) mod 11 == 0
...
ISBN_FORMAT = r'^\d-?\d{3}-?\d{5}-?[\dX]$'


def is_valid(isbn: str) -> bool:
    """
    Check if the given string is a valid isbn number
    """
    return sum(
        coefficient * digit
        for coefficient, digit
        in enumerate(
            (
                int(digit)
                if digit.isdigit()
                else VERIFICATION_PROCESS
                for digit
                in isbn.replace('-', '')
            ), 1
        )
    ) % CHECK_MOD == 0 if re.match(
        ISBN_FORMAT,
        isbn
    ) else False


def is_valid1(isbn: str) -> bool:
...
def is_valid_format(format_string: str) -> Callable[[str], bool]:
    ...
    return lambda isbn: re.match(format_string, isbn)
